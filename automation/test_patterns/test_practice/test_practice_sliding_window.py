import pytest
from automation.resources.patterns.sliding_window_data import SLIDING_WINDOW_MAX_SUBARRAY_OF_SIZE_K_TESTS, \
...
from patterns.sliding_window import PracticeSlidingWindow
...
class TestPracticeSlidingWindow(TestSlidingWindow):
    ...
    def test_longest_substring_with_same_letters_after_replacement(self, input_string, k, output):
        try:
            super().test_longest_substring_with_same_letters_after_replacement(input_string, k, output)
        except NotImplementedError:
            assert True
