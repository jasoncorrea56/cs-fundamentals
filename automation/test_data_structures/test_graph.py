import pytest
from automation.resources.data_structures.graph_data import GRAPH_ADJACENCY_LIST_OUTPUT, GRAPH_ADJACENCY_MATRIX_OUTPUT,\
    GRAPH_UNION_FIND_FIND_TESTS, GRAPH_UNION_FIND_IS_CONNECTED_TESTS, GRAPH_UNION_FIND_UNION_TESTS, \
    GRAPH_PROBLEM_NUMBER_OF_ISLANDS_2_TESTS
from data_structures.graph import UndirectedGraphVertex, UndirectedGraph, UnionFind, GraphProblems
...
class TestGraph(object):
    ...
    def test_undirected_graph_add_vertices(self):
        result = self.undirected_graph.add_vertices([self.node_a, self.node_b, self.node_c, self.node_d, self.node_e])
        assert result is True
