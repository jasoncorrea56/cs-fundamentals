...
router = APIRouter(
    prefix="/data-structures/graph/union-find",
    tags=["Data Structures - Graph (Union-Find) Practice"],
)

_submit: Callable[[MethodsOnly], Awaitable[JSONResponse]] = make_submit_handler_from_matrix(
    key="ds.graph.union_find",
    success_message="All Graph Union-Find data structure tests executed successfully.",
)


@router.post("/submit")
async def submit_graph_union_find_practice(payload: MethodsOnly) -> JSONResponse:
