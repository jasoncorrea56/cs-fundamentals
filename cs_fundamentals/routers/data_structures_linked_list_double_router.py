...
router = APIRouter(
    prefix="/data-structures/linked-list-double",
    tags=["Data Structures - Doubly Linked List Practice"],
)

_submit: Callable[[MethodsOnly], Awaitable[JSONResponse]] = make_submit_handler_from_matrix(
    key="ds.linked_list_double",
    success_message="All Doubly Linked List data structure tests executed successfully.",
)


@router.post("/submit")
async def submit_double_linked_list_practice(payload: MethodsOnly) -> JSONResponse:
